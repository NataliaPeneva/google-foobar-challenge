...
def solution(string):
    ...
    for symbol in modified_string:
        if symbol == ">":
            for i in range((modified_string.index(symbol) + 1), (len(modified_string))):
                if modified_string[i] == "<":
                    count_salutes += 2
            modified_string = modified_string.replace(symbol, "-", 1)

    return count_salutes
